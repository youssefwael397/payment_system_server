const { Branch,Manager, Sales, Sequelize, sequelize } = require("../models/index");
...
const lockBranchById = async (id) => {
  ...
  try {
    ...
    console.log("branchRepo lockBranchById error: " + error);
  }
};
...
const updateLogoImage = async ({ branch_id, logo }) => {
  try {
    ...
    let updated_branch = await Branch.findOne({
      where: { branch_id: branch_id },
    });
    return updated_branch;
  } catch (error) {
