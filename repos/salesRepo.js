...
const updateSalesNationalImages = async (
  id,
...
const getSalesByEmail = async (email) => {
  try {
    const sales = await Sales.findOne({
      where: { email: email, isLock: { [op.not]: true } },
    });
    return sales;
  } catch (error) {
...
const ResetPassword = async ({ id, password }) => {
  try {
    ...
    let updated_sales = await Sales.findOne({
      where: { sales_id: id },
    });
    return updated_sales;
  } catch (error) {
    console.log("salesRepo ResetPassword error: " + error);
  }
};
